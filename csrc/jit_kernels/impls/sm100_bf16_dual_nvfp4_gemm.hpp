...
static void __instantiate_kernel() {{
    ...
    >);
        ...
        args.compiled_n, args.compiled_k,
        args.config.block_m, args.config.block_n, args.config.block_k,
        args.num_groups,
        args.config.swizzle_a_mode, args.config.swizzle_ab_mode, args.config.swizzle_cd_mode,
    ...
    static void launch_impl(const deep_gemm::KernelHandle& kernel,
                            const deep_gemm::LaunchConfigHandle& config, Args args) {
        DG_CUDA_UNIFIED_CHECK(launch_kernel(kernel, config,
            args.grouped_layout, args.weight_global_scales,
            args.shape_m, args.shape_n, args.shape_k,
            args.tensor_map_a, args.tensor_map_b,
            args.tensor_map_sfb, args.tensor_map_cd));
    }
};
...
static void sm100_m_grouped_bf16_dual_nvfp4_gemm_contiguous(
    ...
    const auto tensor_map_cd = deep_gemm::make_tma_cd_desc(
        d, m, n, std::min(config.block_m, 128), config.swizzle_cd_mode / 2,
        static_cast<int>(d.stride(0)), 1, config.swizzle_cd_mode);

    // SFB rows cover 64 K elements each, matching one 128x4 atom.
    const auto tensor_map_sfb = deep_gemm::make_tma_2d_desc(
        sfb, n, (k / 64) * num_groups, config.block_n, config.block_k / 64,
        n, 0);

    const SM100BF16DualNVFP4GemmRuntime::Args args = {
        .shape_m = m, .shape_n = n, .shape_k = k, .num_groups = num_groups,
