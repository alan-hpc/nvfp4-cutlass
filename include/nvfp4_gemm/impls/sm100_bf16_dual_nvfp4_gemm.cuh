...
template <uint32_t SHAPE_N, uint32_t SHAPE_K,
          ...
          uint32_t kNumGroups,
          uint32_t kSwizzleAMode, uint32_t kSwizzleABMode, uint32_t kSwizzleCDMode,
          uint32_t kNumStages,
          uint32_t kNumTransformWarps,
...
sm100_bf16_dual_nvfp4_gemm_impl(int* grouped_layout,
    ...
    DG_STATIC_ASSERT(SMEM_A_SIZE_PER_STAGE % 1024 == 0 and SMEM_A0_SIZE_PER_STAGE % 1024 == 0 and
                     SMEM_B_SIZE_PER_STAGE % 1024 == 0, "A/B shared memory must be 1024 B aligned");
    ...
    auto scheduler = deep_gemm::sched::Scheduler<
        kGemmType, BLOCK_M, BLOCK_N, kNumGroups, 1, false, kNumSMs>(
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                const uint32_t m_idx = scheduler.template get_global_idx<
                    (kGemmType == GemmType::MGroupedMasked), deep_gemm::sched::IndexType::MN>(
                    shape_m, BLOCK_M, m_block_idx);
                const uint32_t n_idx = scheduler.template get_global_idx<
                    true, deep_gemm::sched::IndexType::MN>(shape_n, BLOCK_N, n_block_idx, m_block_idx);
                ...
                const uint32_t sfb_k_idx = scheduler.template get_global_idx<
                    true, deep_gemm::sched::IndexType::SF_K>(
                    shape_sfb_k, 1, k_idx / UMMA_K, m_block_idx);
    ...
    // ==================================================================
        ...
        auto instr_desc = cute::UMMA::make_instr_desc_block_scaled<
            fp4_dtype_t, fp4_dtype_t, float, cutlass::float_ue4m3_t,
            UMMA_M, UMMA_N, cute::UMMA::Major::K, cute::UMMA::Major::K>();
        auto sf_desc = deep_gemm::mma::sm100::make_sf_desc(nullptr);

        DG_STATIC_ASSERT(kNumStages <= 32, "Too many stages");
        auto a0_desc = deep_gemm::mma::sm100::make_umma_desc<
            cute::UMMA::Major::K, BLOCK_M, BLOCK_K, kSwizzleABMode>(
            reinterpret_cast<fp4_dtype_t*>(smem_a0[0]), 0, 0);
        auto a1_desc = a0_desc;
        auto b_desc = deep_gemm::mma::sm100::make_umma_desc<
            cute::UMMA::Major::K, BLOCK_N, BLOCK_K, kSwizzleABMode>(
            reinterpret_cast<fp4_dtype_t*>(smem_b[0]), 0, 0);
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (cute::elect_one_sync()) {
                    ...
                    auto issue_pass = [&](const uint32_t& a, const uint32_t& a_base_lo,
                                          const uint32_t& sfa_tmem_base, const bool& accumulate) {
                        auto desc_a = a0_desc;
                        ...
                        ptx::SM100_MMA_MXF4NVF4_SS::fma(
                            desc_a, desc_b, accum_stage_idx * UMMA_N,
                            accumulate, runtime_desc,
    ...
    // ==================================================================
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            const auto base_m_idx = scheduler.template get_global_idx<
                (not deep_gemm::is_m_grouped_contiguous(kGemmType)), deep_gemm::sched::IndexType::MN>(
                shape_m, BLOCK_M, m_block_idx);
            ...
            (smem_cd, tma_stage_idx, accum_stage_idx * UMMA_N,
             ...
             global_scale, epilogue_warp_idx, lane_idx,
