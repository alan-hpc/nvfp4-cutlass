...
inline uint8_t quantize_e2m1_code(float x) {
    ...
    int idx = 0;
    for (int i = 0; i < 7; ++i)
        if (mag > mid[i]) idx = i + 1;
    for (int i = 0; i < 7; ++i)
        if (mag == mid[i]) idx = tie[i];
    if (std::isnan(mag) || std::isinf(mag)) idx = 7;
...
inline void decompose_block(const float* x, uint8_t* q0_codes, uint8_t* q1_codes,
                            uint8_t& s0_code, uint8_t& s1_code) {
    float amax = 0.0f;
...
inline void reconstruct_block(const uint8_t* q0_codes, const uint8_t* q1_codes,
                              uint8_t s0_code, uint8_t s1_code, float* out) {
...
inline void quantize_weight(const std::vector<float>& w, int num_experts, int n, int k,
                            std::vector<uint8_t>& q_codes,   // (E, N, K)
                            std::vector<uint8_t>& sf_codes,  // (E, N, K/16)
...
inline std::vector<float> reference_gemm(const std::vector<float>& a,
    ...
    for (int e = 0; e < num_experts; ++e) {
        ...
        for (int row = 0; row < m; ++row) {
            if (expert_of_row[row] != e) continue;
...
struct Rng {
    ...
    float normal() {
        ...
        float s = 0.0f;
        for (int i = 0; i < 4; ++i) s += static_cast<float>(next() >> 8) / 16777216.0f;
