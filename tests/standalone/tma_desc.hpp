...
#define CU_CHECK(expr)                                                                                                                    \
    ...
    {                                                                                                                                     \
        ...
        {                                                                                                                                 \
            ...
            std::fprintf(stderr, "%s:%d: driver error %d (%s) in: %s\n", __FILE__, __LINE__, static_cast<int>(_r), _s ? _s : "?", #expr); \
            std::exit(1);                                                                                                                 \
        }                                                                                                                                 \
    } while (0)
...
#define CUDA_CHECK(expr)                                                                                                                         \
    ...
    {                                                                                                                                            \
        ...
        if (_r != cudaSuccess)                                                                                                                   \
        {                                                                                                                                        \
            std::fprintf(stderr, "%s:%d: cuda error %d (%s) in: %s\n", __FILE__, __LINE__, static_cast<int>(_r), cudaGetErrorString(_r), #expr); \
            std::exit(1);                                                                                                                        \
        }                                                                                                                                        \
    } while (0)
...
inline CUtensorMap make_2d(CUtensorMapDataType dtype, void* ptr, uint64_t gmem_inner, uint64_t gmem_outer, uint32_t box_inner, uint32_t box_outer, uint64_t gmem_outer_stride_bytes, int swizzle_mode)
{
    CUtensorMap      map{};
