...
void transform_test_kernel(const __grid_constant__ cute::TmaDescriptor tma_a,
    ...
    if (threadIdx.x < kTransformThreads) {
        transform::transform_a_tile<
            BLOCK_M, BLOCK_K, kSwizzleA, kSwizzleA0,
            kTransformThreads, transform::ScalePolicy::DerivedDiv8>(
            smem_a, smem_a0, smem_a1, smem_sfa0, smem_sfa1, threadIdx.x);
    }
    __syncthreads();
...
int main() {
    ...
    for (auto& v : a_host) v = rng.normal();
    ...
    for (size_t i = 0; i < a_host.size(); ++i) a_bf16[i] = __float2bfloat16(a_host[i]);
    for (size_t i = 0; i < a_host.size(); ++i) a_host[i] = __bfloat162float(a_bf16[i]);
    ...
    auto tma_a = tma_test::make_2d(CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, d_a,
                                   BLOCK_K, BLOCK_M, kSwizzleA / 2, BLOCK_M,
                                   BLOCK_K * 2, kSwizzleA);
    ...
    for (int m = 0; m < BLOCK_M; ++m) {
        for (int b = 0; b < kSFPerRow; ++b) {
            ...
            if (got_s0 != s0 || got_s1 != s1) {
                ...
                if (first_bad_row < 0) first_bad_row = m;
            }
                ...
                if (got_q0[i] != q0[i] || got_q1[i] != q1[i]) {
                    if (++code_mismatch <= 5)
                        std::printf("  code mismatch m=%d k=%d: q0 %u vs %u, q1 %u vs %u\n",
                                    m, col, got_q0[i], q0[i], got_q1[i], q1[i]);
                    if (first_bad_row < 0) first_bad_row = m;
                }
            }
